...
def test_update_board__dig(game):
    board, game_over = update_board_use_case(game.slug, 0, 0, "dig")
...
def test_update_board__dig__empty(game):
    board, game_over = update_board_use_case(game.slug, 2, 0, "dig")
    assert not game_over
...
def test_update_board__flag(game):
    board, game_over = update_board_use_case(game.slug, 0, 0, "flag")
    assert not game_over
...
def test_update_board__dig_bomb(game):
    board, game_over = update_board_use_case(game.slug, 0, 1, "dig")
    assert game_over
